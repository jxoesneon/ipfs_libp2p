...
class YamuxSession implements Multiplexer, core_mux.MuxedConn, Conn {
  ...
  Future<core_mux.MuxedStream> openStream(Context context) async {
    ...
    try {
      ...
      // Do not block on the remote's ACK. go-yamux and js-yamux both make
      // the stream usable once SYN is sent; the acceptor's ACK is
      // piggybacked on its first outgoing frame, which often only happens
      // in response to our own first write (e.g. multistream-select
      // negotiation). Waiting here deadlocks lazy-ACK implementations.
      await stream.open();
